 ...
 public:
  explicit VeriSGDSolver(const SolverParameter& param, const LayerParameter &extra_param)
      : Solver<Dtype>(param), extra_layer_params_(extra_param) {
        ...
        /*********************************
         **   BUILDING TRAINING NET     **
         *********************************/
        LOG(INFO) << "Loading train/shadow train network";
        DATA_LAYER_IDX = 0;
        LOG(INFO) << "Data layer name: " << this->net_->layers()[DATA_LAYER_IDX]->layer_param().name();
        NetParameter train_net_param;
        LOG(INFO) << "Reading network parameters from " << param.train_net();
        ReadProtoFromTextFile(param.train_net(), &train_net_param);
        LOG(INFO) << "Training input: " << train_net_param.layers(DATA_LAYER_IDX).layer().source();
        ...
        LOG(INFO) << "Looking for feature layer (relu5)...";
        FEATURE_LAYER_ID = FindLayer("relu5");
        CHECK_GE(FEATURE_LAYER_ID, 0);
        LOG(INFO) << "Found feature layer, with id " << FEATURE_LAYER_ID;

        LOG(INFO) << "Looking for feature blobs...";
        //Setup loss layer
        ...
        LOG(INFO) << "Feature blob for both training and shadow net exists";
        // ID of the last layer
        TOP_LAYER_ID = this->net_->layers().size() - 1;

        LOG(INFO) << "Top layer ID: " << TOP_LAYER_ID;
        LOG(INFO) << "Top layer name: " << this->net_->layers()[TOP_LAYER_ID]->layer_param().name();

        CHECK_EQ(this->net_->top_vecs()[DATA_LAYER_IDX].size(), 2);
        CHECK_EQ(this->net_->top_vecs()[TOP_LAYER_ID].size(), 0);

        LOG(INFO) << "Looking for label blobs...";
        ...
        LOG(INFO) << "num: " << label_vec1->num() << ", count: " << label_vec1->count() << ", dim: "<< label_vec1->count()/label_vec1->num();
        LOG(INFO) << "num: " << label_vec2->num() << ", count: " << label_vec2->count() << ", dim: "<< label_vec2->count()/label_vec2->num();


        LOG(INFO) << "Building VerificationLossLayer...";
        ...
        LOG(INFO) << "Building training net (using VerificationLossLayer) done";
        ...
        LOG(INFO) << "Loading test/shadow test network";
        //Build test accuracy
        NetParameter test_net_param;
        LOG(INFO) << "Reading network parameters from " << param.test_net();
        ...
        LOG(INFO) << "Testing input: " << test_net_param.layers(DATA_LAYER_IDX).layer().source();
        ...
        LOG(INFO) << "Copying data ptr test network -> shadow test network";
        shadow_test_data_layer->CopyDataPtrFrom(*test_data_layer);
        // Shadow network uses channel 1, while test network uses channel 0
        shadow_test_data_layer->SetOutputChannel(1);

        LOG(INFO) << "Building VerificationAccuracyLayerSiamese";
        ...
        LOG(INFO) << "VerificationAccuracyLayerSiamese built";
