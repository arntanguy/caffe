...
/* ContrastiveLossLayer
  Compute the contrastive loss between the two inputs.
  d = \sum_i (a_i - b_i)^2
  loss = 1/2 (y d + (1-y) \max(margin-d, 0))
  the gradient is propagated to a and b
*/
template <typename Dtype>
class ContrastiveLossLayer : public LossLayer<Dtype> {
 public:
  ...
  // The ContrastiveLossLayer we can backpropagate to a and b but not y
  virtual inline bool AllowForceBackward(const int bottom_index) const {
...
class SiameseAccuracyLayer : public Layer<Dtype> {
 ...
 protected:
  ...
  Dtype correct_genuine;
  Dtype incorrect_genuine;
  Dtype threshold;

  Dtype average_distance_genuine_ra;
  Dtype average_distance_impostor_ra;
  Dtype number_genuine_ra;
  Dtype number_impostor_ra;
};
